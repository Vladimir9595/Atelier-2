...
Install () {
	...
	sudo apt install apache2 php -y php-fpm -y mariadb-server -y mariadb-client -y phpmyadmin -y php-xdebug -y;
...
ConfigApache () {
    ...
    echo "--------------------------------------------------";
    echo "Configuration d'un virtual host vers /var/www/Laravel_project";
    echo "--------------------------------------------------";

    sudo rm -rf /var/www/Laravel_project;
	sudo mkdir /var/www/Laravel_project;

    portfolio=/etc/apache2/sites-available/laravel.conf;

    sudo touch ${portfolio};

    echo "<VirtualHost *:80>" | sudo tee -a ${portfolio};
    echo "" | sudo tee -a ${portfolio};
	echo "          ServerName dev.laravel.com" | sudo tee -a ${portfolio};
    echo "          ServerAdmin webmaster@localhost" | sudo tee -a ${portfolio};
    echo "          DocumentRoot /var/www/Laravel_project/public/" | sudo tee -a ${portfolio};
    echo "" | sudo tee -a ${portfolio};
	...
	echo "" | sudo tee -a ${portfolio};
    echo "          <Directory /var/www/Laravel_project/public/>" | sudo tee -a ${portfolio};
...
Laravel () {
    ...
    sudo composer create-project laravel/laravel /var/www/Laravel_project_test;
    sudo chown ubuntu:ubuntu /var/www/Laravel_project_test && sudo chown ubuntu:ubuntu /var/www/Laravel_project_test/*;
    cd /var/www/Laravel_project_test;
